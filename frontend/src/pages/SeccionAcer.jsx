...
export default function SeccionAcer() {
  const [portatilesAcer, setPortatilesAcer] = useState([]);
  ...
  useEffect(() => {
    ...
    const obtenerProductos = async () => {
      ...
      const response = await obtenerProductosMarcaTipo("acer", "portatil", 8);
      ...
      setPortatilesAcer(productosPorPagina);
    };
  ...
  //Las consultas pueden tardar, porque se hace un condicion, que si portatilesAcer ya tienen datos entonces muestre la interfaz, de lo contrario dira cargando
  return portatilesAcer ? (
    <main className="pt-8 pb-16 ">
      ...
      <section className="">
        ...
        {portatilesAcer.map((portatilAcer, index) => (
          <>
            {console.log(portatilAcer.id)}
            <div key={index} className="flex">
              ...
              >
                ...
                <Slider sliderId={index} data={portatilAcer} />
              </div>
            </div>
            <div className="h-16" />
          </>
